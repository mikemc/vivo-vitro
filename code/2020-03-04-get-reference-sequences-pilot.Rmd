...
# Path to save the reference sequences
ref_path <- file.path(Sys.getenv("DATA_PATH"), "references", "pilot")
dir.create(file.path(ref_path, "genomes-finished"), recursive = TRUE)
```
...
write_lines(fasta, file.path(ref_path, "16s-sequences.fasta"))
```
...
tb <- inoculum_strains %>%
  ...
  mutate(
    ...
    fn = file.path(ref_path, "genomes-finished", glue("{accession}.{ftype}")),
...
```{r}
fns <- list.files(file.path(ref_path, "genomes-draft"), pattern = "fsa_nt.gz",
    full.names = TRUE)
ftb <- tibble(fn = fns) %>%
  mutate(
...
```{r}
fns <- list.files(file.path(ref_path, "genomes-finished"), pattern = "fasta",
    full.names = TRUE) %>%
  c(ftb$new_fn)
all_fn <- file.path(ref_path, "all-genomes.fasta")
...
fasta_out <- file.path(ref_path, "all-genomes-barrnap.fasta")
gff_out <- file.path(ref_path, "all-genomes-barrnap.gff3")
...
the strain.
```{r}
zymo <- file.path(ref_path, "zymo-mock-16s.fasta") %>%
  Biostrings::readDNAStringSet()
names(zymo)
