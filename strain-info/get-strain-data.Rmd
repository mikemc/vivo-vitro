...
```{r}
strains <- read_csv(here("strain-info", "strains-uniprot.csv"),
...
OK! Now time to make a strain data table (consider also adding genome sizes)
and save.
...
gtdb2 <- gtdb1 %>%
  select(contains("taxid"), accession, 
    ...
    contains("taxonomy"), genome_size, gtdb_genome_representative) %>%
...
all.equal(tb$reference_genome, tb$ncbi_genbank_assembly_accession)
tb0 <- tb %>%
  select(strain_group, ncbi_organism_name, ncbi_taxid, ncbi_species_taxid,
    gram_stain, sporulation, ssu_count, 
    reference_genome, reference_genome_size = genome_size,
    gtdb_genome_representative, contains("taxonomy")) %>%
  mutate(across(c(contains("taxid"), ssu_count, reference_genome_size), 
      as.integer))
```
