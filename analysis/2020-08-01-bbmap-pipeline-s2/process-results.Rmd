...
```{r}
dna_sam <- here("output/sample-data", "dna-sample-data.Rds") %>% readRDS
```
...
```{r}
saveRDS(mat, here("output", "s2", "count-matrix.Rds"))
```

## Unmapped reads

Separately, save a table with the number of unmapped reads,

```{r}
unmapped <- path(out_path, "unmapped-stats.tsv") %>%
  read_tsv(col_types = "cii") %>%
  mutate(well = str_replace(well, "(?<=[A-H])0", "")) %>%
  left_join(dna_sam %>% select(well, dna_sample_id), by = "well") %>%
  transmute(
    sample_id = str_c("S2_", dna_sample_id),
    unmapped_reads, 
    unmapped_pairs
  ) %>%
  arrange(sample_id) %>%
...
```{r}
write_csv(unmapped, here("output", "s2", "unmapped-reads.csv"))
```
