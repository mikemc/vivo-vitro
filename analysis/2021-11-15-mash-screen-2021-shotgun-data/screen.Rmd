...
y <- file_sets %>%
  ...
  mutate(
    ...
    out_file = str_glue('{center_id}-{specimen_type_mod}-d{dilution_power}-screen-gtdb.tsv'),
    command_mash = str_glue('mash screen -p {n_threads} -v 0.00000001 -i 0.8 {msh_file} {file_list} > output/{out_file}'),
    command_sbatch = str_glue('sbatch --exclusive --mem=0 --cpus-per-task={n_threads} --job-name=screen-{out_file} --wrap="{command_mash}"')
  )
```
