...
```{r}
track <- read_csv(
  here("results", "dada2", "a1", "track.csv"),
  col_types = "ciiiiii"
  ) %>%
  glimpse
track %>% pull(sample)
...
```{r}
saveRDS(sam0, here("results", "a1-sample-data.Rds"))
```
...
seqtab <- here("results", "dada2", "a1", "seqtab-nochim.Rds") %>%
  readRDS %>%
  otu_table(taxa_are_rows = FALSE)
sample_names(seqtab) <- sample_names(seqtab) %>%
  rename_samples
tax <- here("results", "dada2", "a1", "taxonomy-silva-v138.Rds") %>%
  readRDS %>%
  tax_table
sam <- readRDS(here("results", "a1-sample-data.Rds")) %>%
...
```{r}
saveRDS(ps, here("results", "a1-phyloseq-silva-v138.Rds"))
```
