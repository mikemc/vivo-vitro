...
```{r}
indexqc <- read_csv(
  here::here("sample-data", "a1-index-qc.csv"),
  col_types = "iccccddd"
  ) %>%
  janitor::clean_names() %>%
  glimpse
...
```{r}
track <- read_csv(
  here("results", "a1", "dada2", "track.csv"),
  col_types = "ciiiiii"
  ) %>%
  glimpse
track %>% pull(sample)
...
```{r}
saveRDS(sam0, here("results", "a1", "a1-sample-data.Rds"))
```
...
seqtab <- here("results", "a1", "dada2", "seqtab-nochim.Rds") %>%
  readRDS %>%
  otu_table(taxa_are_rows = FALSE)
sample_names(seqtab) <- sample_names(seqtab) %>%
  rename_samples
tax <- here("results", "a1", "dada2", "taxonomy-silva-v138.Rds") %>%
  readRDS %>%
  tax_table
sam <- readRDS(here("results", "a1", "a1-sample-data.Rds")) %>%
...
```{r}
saveRDS(ps, here("results", "a1", "a1-phyloseq-silva-v138.Rds"))
```
